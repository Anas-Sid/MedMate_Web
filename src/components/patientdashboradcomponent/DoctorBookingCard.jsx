...
const bookedTimes = bookedAppointments.map((a) => a.time);
  ...
  return (
    <div className="w-[450px] p-6 bg-gradient-to-br from-[#F0F3FF] to-[#B3C1EE] rounded-2xl shadow-lg text-black ml-[580px] -mt-[630px] h-[630px]">
...
<p className="text-sm text-gray-600 mb-3">{doctor?.specialization}</p>
      ...
      <div className="flex justify-between mb-4">
        {nextSixDays.map(({ label, date, shortDate }) => (
          <button
            ...
            className={`text-sm px-3 py-3 rounded-2xl shadow-md flex flex-col items-center h-[65px] w-[55px] ${
              selectedDate === date
  ...
  return (
    <button
      ...
      disabled={isBooked}
      className={`text-sm py-2 rounded-lg shadow-sm transition-all duration-150 ${
        isBooked
    ...
    >
      ...
      <button
  onClick={handleBook}
  className="w-[250px] bg-[#4438CA] text-white font-medium py-2 rounded-full shadow hover:bg-[#2f24a1] transition duration-200 mt-3 ml-[80px]"
>
  Book Appointment
</button>
